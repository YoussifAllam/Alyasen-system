...
from PyQt5.QtCore import Qt, QThread, QUrl, QPoint, QObject, pyqtSignal, pyqtSlot
...
class ProjectAttachmentsDialog(QDialog):
    def __init__(self, p_id, parent=None):
        super().__init__(parent)
        self.p_id = p_id
        self.setWindowTitle(f"عقود المشروع (كود: {self.p_id})")
        self.resize(800, 400)
    ...
    def setup_ui(self):
        ...
        title_text = QLabel(f"عقود المشروع (كود: {self.p_id})")
        ...
        self.info_label = QLabel("جاري تحميل המرفقات...")
    ...
    def load_attachments(self):
        url = f"{BACKEND_BASE_URL}/projects/contracts/?project_id={self.p_id}"

        self.thread = QThread()
        self.worker = ProjectApiWorker("GET", url, payload=None)
    ...
    def handle_api_response(self, response_data):
        ...
        if not results:
            self.info_label.setText("لا يوجد عقود لهذا المشروع.")
