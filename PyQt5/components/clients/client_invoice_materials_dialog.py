...
from ..invoice_generator.generate_invoice import ArabicSalesInvoice
...
class InvoiceInfoWorker(QObject):
    ...
    def __init__(self, invoice_num):
        super().__init__()
        self.url = f"{BACKEND_BASE_URL}/clients/invoice/info/?invoice_num={invoice_num}"
...
class ClientInvoiceMaterialsDialog(QDialog):
    def __init__(self, invoice_num, parent=None):
        ...
        self.setWindowTitle(f"تفاصيل أصناف الفاتورة: {invoice_num}")
        ...
        title_text = QLabel(f"تفاصيل أصناف الفاتورة رقم: {invoice_num}")
        ...
        self.table.setHorizontalHeaderLabels(["اسم الصنف", "كود الصنف","الكمية","الوحدة"])
        ...
        # Fetch both mixture details and invoice info
    ...
    def populate_table(self, response_data):
        ...
        for mixture in results:
            ...
            # Store data for invoice generatio
            print("mixture: ",mixture)
            mixture_info = {
                "id": mixture.get("material_id"),
                "name": mixture.get("material_name", ""),
                "quantity_in_unit": mixture.get("quantity_in_unit", 0),
                "unit": mixture.get("material_unit", ""),
            }
            self.table_data.append(mixture_info)

            items = [
                QTableWidgetItem(mixture_info["name"]),
                QTableWidgetItem(f"{mixture_info['id']}"),
                QTableWidgetItem(f"{mixture_info['quantity_in_unit']:,.2f}"),
        ...
        try:
            ...
            for row, mixture in enumerate(self.table_data):
                ...
                items = [
                    ...
                    mixture["name"],                            # Product name
                    f"{mixture['quantity_in_unit']:,.2f}",      # Quantity
                    f"{mixture['unit']}",                       # Unit
                    "0"                                         # Extra fake column
                ]
            ...
            client_info = self.invoice_info.get("client_info", {})

            # Sanitize client name for filename
            client_name = client_info.get("name") or "عميل"
            ...
            invoice_params = {
                ...
                "rest": f"{self._to_float(self.invoice_info.get('total_amount_payable')):,.2f}",
