...
class WorkersUI(QWidget):
    def __init__(self):
        ...
        self.profile_page = WorkerProfileUI()
    ...
    def create_form_panel(self):
        ...
        header = QLabel("إدارة العمال")
        header.setObjectName("mainHeader")
        subheader = QLabel("إضافة عامل جديد أو البحث عن عامل.")
        subheader.setObjectName("mainSubheader")

        layout.addWidget(header)
        layout.addWidget(subheader)

        form_groupbox = QGroupBox("إضافة عامل جديد")
        form_layout = QVBoxLayout(form_groupbox)
        form_layout.setSpacing(15)

        self.name_input = QLineEdit(placeholderText="اسم العامل")
        ...
        btn_choose_pic = QPushButton("اختيار صورة للعامل")
        ...
        self.btn_add_worker = QPushButton("إضافة العامل")
        self.btn_add_worker.setObjectName("primaryButton")
        self.btn_add_worker.clicked.connect(self.handle_add_worker)
        form_layout.addWidget(self.btn_add_worker)
    ...
    def create_table_panel(self):
        ...
        self.btn_register_attendance = QPushButton("تسجيل حضور")
        self.btn_register_attendance.setObjectName("successButton")
        self.btn_register_attendance.setEnabled(False)
        self.btn_register_attendance.clicked.connect(self.handle_register_attendance)
        actions_layout.addWidget(self.btn_register_attendance)

        # --- NEW BUTTON ---
        self.btn_register_departure = QPushButton("تسجيل انصراف")
        self.btn_register_departure.setObjectName("dangerButton")
        self.btn_register_departure.setEnabled(False)
        self.btn_register_departure.clicked.connect(self.handle_register_departure)
        actions_layout.addWidget(self.btn_register_departure)

        self.btn_show_profile = QPushButton("عرض ملف العامل")
        ...
        headers = ["كود العامل", "اسم العامل", "رقم الهاتف"]
    ...
    def on_selection_changed(self):
        ...
        self.btn_register_attendance.setEnabled(is_selected)
        self.btn_register_departure.setEnabled(is_selected)
    ...
    def handle_show_profile(self):
        ...
        url = f"{BACKEND_BASE_URL}/workers/info/?worker_id={worker_id}"
    ...
    def on_add_success(self, response_data):
        QMessageBox.information(self, "نجاح", "تمت إضافة العامل بنجاح.")
    ...
    def handle_search(self):
        ...
        url = f"{BACKEND_BASE_URL}/workers/workers/?{params}"
        self._start_api_request("GET", url, on_success=self.handle_api_response)
    ...
    def handle_register_attendance(self):
        ...
        if not selected_rows:
            return
        worker_id = self.table.item(selected_rows[0].row(), 0).data(Qt.UserRole)
        payload = {"worker_id": worker_id, "transaction": "in"}
        url = f"{BACKEND_BASE_URL}/workers/attendance/"
        self._start_api_request("POST", url, payload=payload, on_success=self.on_attendance_success)

    def on_attendance_success(self, response_data):
        QMessageBox.information(self, "نجاح", "تم تسجيل الحضور بنجاح.")

    def handle_register_departure(self):
        """Handles the API call to register departure for the selected worker."""
        selected_rows = self.table.selectionModel().selectedRows()
        if not selected_rows:
            return
        worker_id = self.table.item(selected_rows[0].row(), 0).data(Qt.UserRole)
        payload = {"worker_id": worker_id, "transaction": "out"}
        url = f"{BACKEND_BASE_URL}/workers/attendance/"
        self._start_api_request("POST", url, payload=payload, on_success=self.on_departure_success)

    def on_departure_success(self, response_data):
    ...
    def _start_api_request(self, method, url, payload=None, files=None, on_success=None):
        ...
        self.thread = QThread()
        self.worker = WorkerApiWorker(method, url, payload, files)
    ...
    def populate_table(self, workers):
        ...
        for worker in workers:
            ...
            id_item.setData(Qt.UserRole, worker_id)
            items = [
                id_item,
                QTableWidgetItem(worker.get("name", "")),
                QTableWidgetItem(worker.get("phone", "")),
            ]
    ...
    def update_pagination_controls(self):
        ...
        if self.total_count > 0:
            self.page_info_label.setText(f"إجمالي العمال: {self.total_count}")
        else:
    ...
    def _set_loading(self, is_loading):
        ...
        self.btn_add_worker.setDisabled(is_loading)
        self.btn_register_attendance.setDisabled(is_loading)
        self.btn_register_departure.setDisabled(is_loading)
        if is_loading:
    ...
    def show_error_message(self, message):
        ...
        QMessageBox.critical(self, "خطأ من السيرفر", message)
