...
class DeductionApiWorker(QObject):
    """Worker for handling API requests for deductions."""
...
class DeductionDialog(QDialog):
    ...
    def __init__(self, worker_name, worker_id, parent=None):
        ...
        self.setWindowTitle(f"سجل خصومات العامل: {worker_name}")
        ...
        title_text = QLabel(f"سجل خصومات العامل: {worker_name}")
        ...
        form_group = QGroupBox("إضافة خصم جديد")
        ...
        self.add_button.clicked.connect(self.handle_add_deduction)

        # --- UPDATED: New Form Layout ---
        ...
        self.table.setHorizontalHeaderLabels(["تاريخ الخصم", "المبلغ", "السبب", ""])
    ...
    def handle_add_deduction(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "خطأ", "الرجاء إدخال مبلغ خصم صحيح وموجب.")
            return
        ...
        payload = {
            ...
            "deduction_date": self.date_input.date().toString("yyyy-MM-dd"),
            "deduction_reason": self.reason_input.text().strip(),
            "username": username,
            "deduction_amount": amount,
        }
        url = f"{BACKEND_BASE_URL}/workers/deduction/"
        self._start_api_request("POST", url, payload, self.on_add_success)

    def on_add_success(self, response):
        QMessageBox.information(self, "نجاح", "تم تسجيل الخصم بنجاح.")
    ...
    def fetch_deductions(self):
        url = f"{BACKEND_BASE_URL}/workers/deduction/?worker_id={self.worker_id}"
    ...
    def handle_api_response(self, response_data):
        """Handles the full API response, including pagination links."""
    ...
    def populate_table(self, results):
        ...
        for item in results:
            ...
            deduction_id = item.get("id")
            date_item = QTableWidgetItem(item.get("deduction_date", ""))
            date_item.setTextAlignment(Qt.AlignCenter)
            date_item.setData(Qt.UserRole, deduction_id)
            amount_item = QTableWidgetItem(f"{item.get('deduction_amount', 0):,.2f}")
            amount_item.setTextAlignment(Qt.AlignCenter)
            reason_item = QTableWidgetItem(item.get("deduction_reason", ""))
            ...
            delete_button.clicked.connect(lambda ch, did=deduction_id: self.handle_delete_deduction(did))
    ...
    def update_pagination_controls(self):
        ...
        if self.total_count > 0:
            ...
            self.page_info_label.setText("لا توجد خصومات")

    def handle_delete_deduction(self, deduction_id):
    ...
    def _start_api_request(self, method, url, payload=None, on_success=None):
        ...
        self.thread = QThread()
        self.worker = DeductionApiWorker(method, url, payload)
