...
class KpiApiView(APIView):
    def get(self, request: Request, format=None):
        kpi_data = kpi_tasks.get_kpi_data()

        return Response({"status": "success", "data": kpi_data}, status=HTTP_200_OK)


class ExpensesGraphApiView(APIView):
    def get(self, request: Request, format=None):
        expenses_analysis = expenses_graph_tasks.ExpensesAnalysisService.get_expenses_breakdown()

        return Response({"status": "success", "data": expenses_analysis}, status=HTTP_200_OK)
...
class InventoryMaterialsLevelApiView(APIView):
    def get(self, request: Request, format=None):
        lowest_M_in_quantity = inventory_levels_tasks.MaterialInventoryService.get_lowest_quantity_materials()

        return Response({"status": "success", "data": lowest_M_in_quantity}, status=HTTP_200_OK)
...
class PerformanceGraphApiView(APIView):
    def get(self, request: Request, format=None):
        ...
        return Response({"status": "success", "data": performance_analysis}, status=HTTP_200_OK)
...
class UsersStutsApiView(APIView):
    def get(self, request: Request, format=None):
        ...
        return Response({"status": "success", "data": users_analysis}, status=HTTP_200_OK)
