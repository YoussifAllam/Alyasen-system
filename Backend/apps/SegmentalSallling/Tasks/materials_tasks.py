from apps.Mixtures.models import MaterialWarehouse
...
def check_material_qty_availability(material_instance: MaterialWarehouse, req_qty_of_material: float):

    if material_instance.quantity_in_unit < req_qty_of_material:
        return False, ("لا توجد كمية كافية في المخزن ")

    else:
        return True, " "


def create_material_production_from_invoice(invoice_instance: Invoice):
    """
    ...
    with transaction.atomic():
        ...
        invoice_materials = SegmentalInvoiceMaterials.objects.select_related("material").filter(
