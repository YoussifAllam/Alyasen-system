...
from ..models import MaterialSupplier, SupplierInvoice, InvoicePayment, InvoiceMaterial
...
def get_suppliers(request: Request):
    ...
    @cached_as(MaterialSupplier, extra=cache_key, timeout=3600)
    def _get_filtered_transactions():

        if search_query:
            return MaterialSupplier.objects.filter(
                Q(name__icontains=search_query) | Q(phone__icontains=search_query)
            )

        return MaterialSupplier.objects.all()

    result = _get_filtered_transactions()
    return result


def get_supplier_instance(supplier_id: int) -> MaterialSupplier:
    try:
        return MaterialSupplier.objects.get(id=supplier_id)
