...
def get_client_statement(client_id, start_date, end_date):
    ...
    past_invoices = (
        ClientInvoice.objects.filter(
            client=client, invoice_date__lt=start_date
        ).aggregate(Sum("invoice_total_amount"))["invoice_total_amount__sum"]
        or 0
    )
    past_payments = (
        InvoicePayment.objects.filter(
            client_fk=client, payment_date__lt=start_date
    ...
    invoices = ClientInvoice.objects.filter(
        client=client, invoice_date__range=[start_date, end_date]
    )
    payments = InvoicePayment.objects.filter(
        client_fk=client, payment_date__range=[start_date, end_date]
    )
    ...
    transactions = sorted(chain(invoices, payments), key=attrgetter("date"))
