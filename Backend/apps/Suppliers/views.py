...
class SupplierApiView(APIView):
    def get(self, request: Request, format=None):
        ...
        response_data = pagenator(filtred_transactions, request, OutputSerializers.SupplierSerializer)
...
class InovicePaymentApiView(APIView):
    def post(self, request: Request, format=None):
        ...
        celery_tasks.create_supplier_payment_record.delay(supplier_id, payment_amount, notes)
    ...
    def get(Self, request: Request, format=None):
        ...
        response_data = pagenator(payments_instances, request, OutputSerializers.InvoicePaymentsSerializer)
...
class SupplierInovicesApiView(APIView):
    ...
    def post(self, request: Request, format=None):
        ...
        return Response(
            {"status": "success", "invoice_number": invoice_instance.invoice_number}, status=HTTP_200_OK
        )
    ...
    def delete(self, request: Request, format=None):
        ...
        tranaction = (
            f"تم حذف فاتورة رقم {invoice_instance.invoice_number} من المورد {invoice_instance.supplier.name}"
        )
        username = request.data["username"]
        create_transaction_log.delay(transaction_data=tranaction, username=username)
...
class InvoiceMaterialsApiView(APIView):
    def post(self, request: Request, format=None):
        ParamsSerializers.validate_serializer(ParamsSerializers.inviceMaterialsPSerializer, data=request.data)
    ...
    def delete(self, request: Request, format=None):
        ...
        material_instance = selectors.get_specific_invoice_materials_instance(material_id)
...
class MoveInvoiceMaterialsToWarehouseAPIView(APIView):

    @transaction.atomic
    def post(self, request: Request):
        invoice_num = request.data["invoice_number"]
        invoice_instance = selectors.get_invoices_instance(invoice_num)
        if invoice_instance.is_moved_to_warehouse:
            return Response({"خطأ": "هذه الفاتوره تم نقلها الي المخزن من قبل"})

        invoice_materials = selectors.get_invoice_materials_instance(invoice_num)
        materials_tasks.validate_materials_exist(invoice_materials, invoice_num)

        moved_materials = materials_tasks.process_materials(invoice_materials)
        # services.update_supplier_balance(invoice_instance)
        invoice_instance.is_moved_to_warehouse = True
        invoice_instance.save()

        transaction = f"تم أضافة الفاتوره رقم {invoice_num} الي المخزن"
        create_transaction_log.delay(request.data["username"], transaction)
        return self._success_response(moved_materials)

    def _success_response(self, moved_materials):
        return Response(
            {
                "status": "success",
                "message": "Materials moved to warehouse successfully",
                "data": {
...
class InvoiceInfoView(APIView):
    ...
    def patch(self, request: Request, format=None):
        ...
        services.update_supplier_balance(float(invoice_total_amount), float(paid_amount), invoice_instance.supplier)
